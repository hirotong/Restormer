...
from basicsr.data.data_util import (
    ...
    paired_paths_from_textir_folder,
)
...
# @DATASET_REGISTRY.register()
class TextDeblurPairedDataset(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(TextDeblurPairedDataset).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt["io_backend"]
        ...
        if self.io_backend_opt["type"] == "lmdb":
            ...
            self.paths = paired_paths_from_textir_folder(
                [self.lq_folder, self.gt_folder], ["lq", "gt"], self.filename_tmpl
            )
    ...
    def __getitem__(self, index):
        ...
        lq_label = extract_label(lq_path)
        ...
        return {"lq": img_lq, "gt": img_gt, "lq_path": lq_path, "gt_path": gt_path, "lq_label": lq_label}

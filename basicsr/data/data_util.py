...
def paired_paths_from_textir_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders. Specific for textir dataset.
    Args:
        folders (list[str]): A list of folder path. The order of list should
            be [input_folder, gt_folder].
        keys (list[str]): A list of keys identifying folders. The order should
    ...
    gt_paths = list(scandir(gt_folder, suffix="png"))

    degra_folders = os.listdir(input_folder)

    paths = []

    for degra_folder in degra_folders:
        input_paths = list(scandir(osp.join(input_folder, degra_folder)))
        ...
        for idx in range(len(gt_paths)):
            ...
            input_path = osp.join(input_folder, degra_folder, input_name)
            assert input_name in input_paths, f"{input_name} is not in " f"{input_key}_paths."
            gt_path = osp.join(gt_folder, gt_path)
            paths.append(dict([(f"{input_key}_path", input_path), (f"{gt_key}_path", gt_path)]))
    return paths
...
def paired_DP_paths_from_folder(folders, keys, filename_tmpl):
    ...
    assert len(folders) == 3, (
        "The len of folders should be 3 with [inputL_folder, inputR_folder, gt_folder]. " f"But got {len(folders)}"
    )
    ...
    for idx in range(len(gt_paths)):
        ...
        paths.append(
            dict(
                [(f"{inputL_key}_path", inputL_path), (f"{inputR_key}_path", inputR_path), (f"{gt_key}_path", gt_path)]
            )
        )
    return paths
